...
class TestInfluxDBCacheRequests(unittest.TestCase):
    ...
    def test_request_ohlc(self):
        with IQFeedHistoryProvider(num_connections=2) as history, \
            ...
            requested_data = history.request_data(BarsInPeriodFilter(ticker=["AAPL", "IBM"], bgn_prd=datetime.datetime(2017, 4, 1), end_prd=end_prd, interval_len=3600, ascend=True, interval_type='s'), sync_timestamps=False, adjust_data=True)
    ...
    def test_request_deltas(self):
        with IQFeedHistoryProvider(num_connections=2) as history, \
            ...
            # test multisymbol request
            requested_data = history.request_data(BarsInPeriodFilter(ticker=["AAPL", "IBM"], bgn_prd=datetime.datetime(2017, 4, 1), end_prd=end_prd, interval_len=3600, ascend=True, interval_type='s'), sync_timestamps=False, adjust_data=False)
    ...
    def test_synchronize_timestamps(self):
        with IQFeedHistoryProvider(num_connections=2) as history, \
            ...
            requested_data = history.request_data(BarsInPeriodFilter(ticker=["AAPL", "IBM"], bgn_prd=datetime.datetime(2017, 4, 1), end_prd=end_prd, interval_len=3600, ascend=True, interval_type='s'), sync_timestamps=True, adjust_data=False)
